---
title: "Hi. I'm Mira."
description: "Mira is an assistant being built around the idea that the agent should be the user interface, not a feature inside one."
---
...
- The agent IS the UI. Every user-facing surface is conversational — including install, errors, and recovery.
- First contact is in chat the user already uses (Discord, Telegram, web), not a terminal.
- The polish layer is generated, not hand-coded.
- Restraint by default — sensible defaults; advanced options on explicit request.
- Reports surface as conversation, not as paths to log files.
- Optional local install for power users, available after they already like the product.
